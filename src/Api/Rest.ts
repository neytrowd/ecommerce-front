        ...
        if (error.response.status) {
            switch (error.response.status) {
                case 500: {
                    ...
                    LoadingHandler.getInstance().errorLoading(
                        error.config?.url,
                    );
                ...
                case 401: {
                    LoadingHandler.getInstance().cancelLoading(
                        error.config?.url,
                    );
                    globalHistory.push(
                        PagesRouting.AuthorizationPages.LoginPage,
                    );
...
async function get<TResponse>(
    ...
    const query =
        params === undefined ? '' : qs.stringify(params, { allowDots: true });
...
async function postBinary<TRequest>(
    url: string,
    data: TRequest,
): Promise<BinaryResponse> {
    const timeOut = startLoading(url);
...
async function deleteRequest<TRequest, TResponse>(
    url: string,
    data: TRequest,
): Promise<TResponse> {
    const lang = window.localStorage.getItem('lang');
